...
public class PointerRender extends Pointer {
	...
	private void calcRadius() {
		if (Float.compare(mPointerRadiusPercentage, 0.0f) == 1) {
			mPointerRadius = MathHelper.getInstance().mul(mParentRadius,
					mPointerRadiusPercentage);
		}

		if (Float.compare(mPointerTailRadiusPercentage, 0.0f) == 1) {
			mPointerTailRadius = MathHelper.getInstance().mul(mParentRadius,
					mPointerTailRadiusPercentage);
		}
	}
	...
	private void calcEndXY() {
		MathHelper.getInstance().calcArcEndPointXY(
				...
				mCenterY,
				mPointerRadius,
				MathHelper.getInstance().add(getCurrentPointerAngle(),
						mStartAngle));
		...
		if (Float.compare(mPointerTailRadiusPercentage, 0.0f) == 1) {
			...
			MathHelper.getInstance().calcArcEndPointXY(mCenterX, mCenterY,
					mPointerTailRadius, tailAgent);
	...
	public void renderTriangle(Canvas canvas) {
		float currentAgent1 = MathHelper.getInstance().add(
				mPointerAngle - FIX_ANGLE, mStartAngle);
		float currentAgent2 = MathHelper.getInstance().add(
				mPointerAngle + FIX_ANGLE, mStartAngle);

		float bX = 0.0f, bY = 0.0f, eX = 0.0f, eY = 0.0f;
		MathHelper.getInstance().calcArcEndPointXY(mTailX, mTailY, mBaseRadius,
				currentAgent1);
