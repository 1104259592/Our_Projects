...
public class XChart implements IRender {
	...
	protected void calcPlotRange() {
		...
		if (null == plotArea)
			return;
		plotArea.setBottom(sub(this.getBottom() - borderWidth / 2,
				mPaddingBottom));
	...
	protected void renderTitle(Canvas canvas) {
		...
		if (null == plotTitle)
			return;
		this.plotTitle.renderTitle(mLeft + borderWidth, mRight - borderWidth,
				mTop + borderWidth, mWidth, this.plotArea.getTop(), canvas);
	}
	 ...
	 * @return 是否有绘制
	 */
	protected boolean drawAnchor(List<AnchorDataPoint> anchorSet, int dataID,
                                 int childID, Canvas canvas, float x, float y, float radius) {
	 ...
	 *            结束颜色
	 */
	public void setBackgroundColor(XEnum.Direction direction, int beginColor,
                                   int endColor) {
		...
		if (beginColor == endColor) {
			...
			if (direction == XEnum.Direction.VERTICAL) {
				linearGradient = new LinearGradient(0, 0, 0, getBottom()
						- getTop(), beginColor, endColor,
						Shader.TileMode.MIRROR);
			} else {
				linearGradient = new LinearGradient(getLeft(), getBottom(),
						getRight(), getTop(), beginColor, endColor,
	...
	protected void renderChartBackground(Canvas canvas) {
		...
		if (this.mBackgroundColorVisible) {
			if (null == mBorder)
				mBorder = new BorderRender();
			if (mShowBorder) {
				mBorder.renderBorder("CHART", canvas, mLeft, mTop, mRight,
						mBottom);
			} else { // 要清掉 border的默认间距
				int borderSpadding = mBorder.getBorderSpadding();
				mBorder.renderBorder("CHART", canvas, mLeft - borderSpadding,
						mTop - borderSpadding, mRight + borderSpadding, mBottom
								+ borderSpadding);
			}
		}
	}
	 ...
	 *            缩放中心点y坐标
	 */
	public void setScale(float xScale, float yScale, float centerX,
			float centerY) {
		mXScale = xScale;
		mYScale = yScale;
		mCenterX = centerX;
	...
	private void scaleChart(Canvas canvas) {
		...
		if (Float.compare(mCenterX, 0.0f) == 1
			...
			// }else{
			// canvas.scale(mScale,
			// mScale,plotArea.getCenterX(),plotArea.getCenterY());
		}
	}

	/**
	 * 激活图表缩放(但注意，图表缩放后，如果有同时激活click事件， 则缩放状态下，点击处理无效。
	 * 
	 */
	...
	/**
	 * Java是无法精确计算小数后面的，激活后会 忽略Java数据计算时的误差。能提高绘制性能，(但饼图类图表慎用)
	 */
