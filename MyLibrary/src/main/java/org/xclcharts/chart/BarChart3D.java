...
public class BarChart3D extends BarChart {
	...
	protected boolean renderHorizontalBar(Canvas canvas) {
		...
		for (int i = 0; i < barNumber; i++) {
			...
			for (int j = 0; j < barValues.size(); j++) {
				...
				setBarDataColor(mBar3D.getBarPaint(), barDataColor, j,
						barDefualtColor);

				float drawBarButtomY = add(sub(barInitY, mul((j + 1), YSteps)),
						labelBarUseHeight / 2);
				drawBarButtomY = sub(drawBarButtomY,
						(barHeight + barInnerMargin) * currNumber);

				// 参数值与最大值的比例 照搬到 y轴高度与矩形高度的比例上来
				double tlen = MathHelper.getInstance().sub(bv,
						dataAxis.getAxisMin());
				float valuePostion = mul(scrWidth,
						div((float) (tlen), valueWidth));

				// 画出柱形
				float topY = sub(drawBarButtomY, barHeight);
				float rightX = add(barInitX, valuePostion);
				mBar3D.renderHorizontal3DBar(barInitX, topY, rightX,
						drawBarButtomY, mBar3D.getBarPaint().getColor(), canvas);

				// 保存位置
				saveBarRectFRecord(i, j, barInitX + mMoveX, topY + mMoveY,
						rightX + mMoveX, drawBarButtomY + mMoveY);

				float labelTopY = sub(drawBarButtomY, barHeight / 2);

				// 在柱形的顶端显示上批注形状
				drawAnchor(this.getAnchorDataPoint(), i, j, canvas, rightX,
						labelTopY, 0.0f);
	...
	protected float get3DBaseOffsetY()// 分类
	{
		...
		double th = MathHelper.getInstance().add(
				MathHelper.getInstance().add(baseOffsetY, baseTickness),
				labelHeight);
	...
	protected boolean renderVerticalBar(Canvas canvas) {
		...
		for (int i = 0; i < barNumber; i++) {
			...
			for (int j = 0; j < barValues.size(); j++) {
				...
				float valuePostion = (float) (MathHelper.getInstance().div(
						tlen, axisRange));
				valuePostion = mul(plotArea.getPlotHeight(), valuePostion);

				// 计算同分类多柱 形时，新柱形的起始X坐标
				float drawBarStartX = sub(add(barInitX, mul((j + 1), XSteps)),
						labelBarUseWidth / 2);
				...
				mBar3D.renderVertical3DBar(drawBarStartX, topY, drawBarEndX,
						barInitY, mBar3D.getBarPaint().getColor(), canvas);
				...
				// 在柱形的顶端显示上批注形状
				drawAnchor(this.getAnchorDataPoint(), i, j, canvas, labelTopX,
						topY, 0.0f);

				// 在柱形的顶端显示上柱形的当前值
				mBar3D.renderBarItemLabel(getFormatterItemLabel(bv), labelTopX,
						topY, canvas);

				// 显示焦点框
				drawFocusRect(canvas, i, j, drawBarStartX, topY, drawBarEndX,
						barInitY);
	...
	protected void drawClipAxisLine(Canvas canvas) {
		...
		case HORIZONTAL:
			// x轴 线 [要向里突]
			dataAxis.renderAxis(canvas, plotArea.getLeft(),
					plotArea.getBottom(), plotArea.getPlotRight(),
					plotArea.getBottom());
